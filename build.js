...
const filePath = path.join(__dirname, 'es6/JyfObjCrafter.js');

console.log(chalk`{blue.underline begin ejs}`);
run(`npx babel JyfObjCrafter.js  --out-dir ejs`)
console.log(chalk`{blue.underline finish ejs}`);

console.log(chalk`{blue.underline begin es6}`);
run(`npx babel JyfObjCrafter.js  --out-dir es6`)
fs.readFile(filePath, 'utf8', (err, data) => {
  ...
  const modifiedData = match[1] + `export default JyfObjCrafter;`;
